    ...
    if (user.role === "admin") {
      ...
      if(existing){
        return res.status(400).json({
          message: "Admin cannot crete duplicate public component name"
        })
      }
    }
...
return res.status(200).json({component})
      ...
      return res.status(500).json({ message: "failed to save component" });
...
try {
   ...
   if (!user || user.role !== "admin") {
     return res.status(404).json({ message: "Only admin can publish" });
   }
...
const nameCheck = `{ ${component.name} }`;
if (!indexContent.includes(nameCheck)) {
...
component.npmPackage = "ganesh-ui-library";
